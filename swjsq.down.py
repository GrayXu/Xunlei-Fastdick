...
account_session = '.swjsq.session.down'
...
class fast_d1ck(object):
    ...
    def renew_xunlei(self):
        ...
        self.load_xl(dt)  # 把renew之后的新dt更新到成员里
        self.save_context(dt)  # 持久化到本地
        return dt
    ...
    def run(self, uname, pwd, save=True):
        ...
        for _vas, _name, _v in [(VASID_DOWN, 'fastdick', 'do_down_accel')]:  # 只提速一种
